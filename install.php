...
if(isset($_POST['submit'])){
	...
	";
    $query['sk_father_module']="
		CREATE TABLE IF NOT EXISTS `sk_father_module` (
		  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
	...
	";
    $query['sk_info']="
		CREATE TABLE IF NOT EXISTS `sk_info` (
		  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
		  `title` varchar(255) NOT NULL,
	...
	";
    $query['sk_manage']="
		CREATE TABLE IF NOT EXISTS `sk_manage` (
		  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
	...
	";
    $query['sk_reply']="
		CREATE TABLE IF NOT EXISTS `sk_reply` (
		  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
	...
	";
    $query['sk_son_module']="
		CREATE TABLE IF NOT EXISTS `sk_son_module` (
		  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
    ...
    $query_info_s="select * from sk_info where id=1";
    $result=mysqli_query($link, $query_info_s);
    if(mysqli_num_rows($result)!=1){
        $query_info_i="INSERT INTO `sk_info` (`id`, `title`, `keyword`, `description`) VALUES(1, 'skbbs', 'sk库', 'sk库');";
        mysqli_query($link,$query_info_i);
        if(mysqli_errno($link)){
            exit("数据库sfk_info写入数据失败请检查相应权限!<a href='install.php'>点击返回</a>");
        }
    }
    $query_manage_s="select * from sk_manage where name='admin'";
    $result=mysqli_query($link, $query_manage_s);
    if(mysqli_num_rows($result)!=1){
        $query_manage_i="INSERT INTO `sk_manage` (`name`, `pw`, `create_time`, `level`) VALUES('admin',md5('{$_POST['manage_pw']}'),now(), 0)";
        mysqli_query($link,$query_manage_i);
        if(mysqli_errno($link)){
            exit("管理员创建失败，请检查数据表sk_manage是否具有写权限!<a href='install.php'>点击返回</a>");
        }
    }
$filename='inc/config.inc.php';
$str_file=file_get_contents($filename);
$pattern="/'DB_HOST',.*?\)/"; //修改config设置 利用正则表达式
...
if(!file_put_contents($filename, $str_file)){//将修改后的STR写入文件
...
<div id="main">
    ...
    <form method="post">
        ...
        <label>数据库用户名(root)：<input class="text" type="text" name="db_user" /></label>
