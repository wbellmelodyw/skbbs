...
$query="select * from sk_father_module where id={$_GET['id']}";
...
$query="select count(*) from sk_content where module_id in({$id_son})";//
$count_all=num($link,$query);
$query="select count(*) from sk_content where module_id in({$id_son}) and time>CURDATE()";
...
<?php include 'inc/header.inc.php'?>
    ...
    <div id="main" class="auto">
        <div id="left">
            ...
            <ul class="postsList">
                <?php
                $query="select sk_content.title,sk_content.id,sk_content.module_id,sk_content.member_id,sk_content.time,sk_content.times,sk_member.name,sk_member.photo,sk_son_module.module_name
from sk_content,sk_member,sk_son_module where
sk_content.module_id in({$id_son}) and
sk_content.member_id=sk_member.id and
sk_content.module_id=sk_son_module.id {$page['limit']}";
                $result_content=execute($link,$query);
                while($data_content=mysqli_fetch_assoc($result_content)){
                    $data_content['title']=htmlspecialchars($data_content['title']);
                    $query="select time from sk_reply where content_id={$data_content['id']} order by id desc limit 1";
                    ...
                    $query="select count(*) from sk_reply where content_id={$data_content['id']}";
                ?>
                <li>
                    <div class="smallPic">
                        <a href="member.php?id=<?php echo str_replace ("0", "", $data_content['member_id']);?>">
                            <img width="45" height="45"src="<?php if($data_content['photo']!=''){echo $data_content['photo'];}else{echo 'style/photo.jpg';}?>">
                        </a>
                    </div>
                    <div class="subject">
                        <div class="titleWrap"><a href=<?php echo "list_son.php?id={$data_content['module_id']}";?>>[<?php echo $data_content['module_name']?>]</a>&nbsp;&nbsp;<h2><a target="_blank"  href="show.php?id=<?php echo $data_content['id']?>"><?php echo $data_content['title']?></a></h2></div>
                        <p>
            ...
            <div class="classList">
                ...
                <ul class="listWrap">
                    ...
                    <li>
                        ...
                        <ul>
                            <?php
                            $query="select * from sk_son_module where father_module_id={$data_father['id']}";
                            $result_son=execute($link, $query);
                            while($data_son=mysqli_fetch_assoc($result_son)){
                                ?>
