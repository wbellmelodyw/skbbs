...
$query="select * from sk_father_module where id={$_GET['id']}";//$_GET['id'];get传递过来的id
$result=execute($link,$query);
if(!mysqli_num_rows($result)){
    skip('father_module.php','error','这条版块信息不存在！');
}
$data=mysqli_fetch_assoc($result);//print_r($data);提取结果集存储数组data
if(isset($_POST['submit'])){
    if(empty($_POST['module_name'])){
        skip("father_module_update.php?id={$_GET['id']}",'error','版块名称不得为空！');
    }
    if(mb_strlen($_POST['module_name'])>66){
        skip("father_module_update.php?id={$_GET['id']}",'error','版块名称不得多余66个字符！');
    }
    if(!is_numeric($_POST['sort'])){
        skip("father_module_update.php?id={$_GET['id']}",'error','排序只能是数字！');
    }
    $_POST=escape($link,$_POST);
    $query="update sk_father_module set module_name='{$_POST['module_name']}',sort={$_POST['sort']} where id={$_GET['id']}";
    execute($link,$query);
...
<div id="main">
    <div class="title" style="margin-bottom:20px;">修改父版块-编辑</div>
    <form method="post">
        <table class="au">
            <tr>
                <td>版块名称</td>
