...
if(isset($_POST['submit'])){
    foreach($_POST['sort'] as $key=>$val){
        ...
        $query[]="update sk_son_module set sort={$val} where id={$key}";
    }
    //print_r($query);exit();
    if(execute_multi($link,$query,$error)){
        skip('son_module.php','ok','排序修改成功！');
    }else{
        skip('son_module.php','ok',$error);
    }
...
<?php include 'inc/header.inc.php'?>
    <div id="main">
        ...
        <table class="list">
            ...
            <?php
            $query="select ssm.id,ssm.sort,ssm.module_name,sfm.module_name father_module_name,ssm.member_id from sk_son_module ssm,sk_father_module sfm where ssm.father_module_id=sfm.id order by sfm.id";
