...
$link=connect();
if(!$member_id=is_login($link)){ //返回会员id
    skip('login.php', 'error', '请登录之后再发帖!');
}
if(isset($_POST['submit'])){
    include 'inc/check_publish.inc.php';
    $_POST=escape($link,$_POST);
    $query="insert into sk_content(module_id,title,content,time,member_id) values({$_POST['module_id']},'{$_POST['title']}','{$_POST['content']}',now(),{$member_id})";
    execute($link, $query);
    ...
    <div id="publish">
        <form method="post">
            <select name="module_id">
                ...
                $query="select * from sk_father_module {$where} order by sort desc";
                $result_father=execute($link, $query);
                while ($data_father=mysqli_fetch_assoc($result_father)){
                    echo "<optgroup label='{$data_father['module_name']}'>";
                    $query="select * from sk_son_module where father_module_id={$data_father['id']} order by sort desc";
