...
function upload($save_path,$custom_upload_max_filesize,$key,$type=array('jpg','jpeg','gif','png')){
    ...
    if($custom_bytes>$phpini_bytes){
        $return_data['error']='你所设的$custom_upload_max_filesize大于PHP配置文件里面的'.$phpini;
    ...
    if(!isset($_FILES[$key]['error'])){//_FILES是二维数组Files[表单input file里面的name][..]
    ...
    if(!is_uploaded_file($_FILES[$key]['tmp_name'])){//is_up...用来验证是否HTTP POST方式上传
    ...
    $arr_filename=pathinfo($_FILES[$key]['name']);//获取文件路径信息
    if(!isset($arr_filename['extension'])){//后缀名
    ...
    $new_filename=str_replace('.','',uniqid(mt_rand(100000,999999),true));//新建随机文件名
    if($arr_filename['extension']!=''){
        $new_filename.=".{$arr_filename['extension']}";
    }
    $save_path=rtrim($save_path,'/').'/';
    if(!move_uploaded_file($_FILES[$key]['tmp_name'],$save_path.$new_filename)){//将临时文件改到指定目录
