...
if(isset($_POST['submit'])){
    ...
    escape($link,$_POST);
    $query="select * from sk_member where name='{$_POST['name']}' and pw=md5('{$_POST['pw']}')";
    $result=execute($link, $query);
    if(mysqli_num_rows($result)==1){
        setcookie('sk[name]',$_POST['name'],time()+$_POST['time']);
        setcookie('sk[pw]',sha1(md5($_POST['pw'])),time()+$_POST['time']);
